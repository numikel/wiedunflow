...
- **ADR-0010** — Secret redaction policy + zero-telemetry contract: 7 binary decisions (pattern-only regex, structlog processor scope, separate `consent.yaml`, per-provider persistence, 9-pattern hard-refuse list, dual-layer zero-telemetry test, editor resolver shlex+which+metachar validation) (2026-04-22; amended 2026-05-05 with §D11 — 12-pattern log redaction catalog: Anthropic/OpenAI/HF/Bearer/Auth/hex + AWS AKIA/AWS Secret/GitHub PAT/GitHub fine-grained PAT/PEM private key, v0.9.6 redaction extension; amended 2026-05-05 with §D12 — path redaction default-ON for logs and `run-report.json` stack_trace, opt-out via `--no-log-redact-paths`, v0.9.7).
...
- **ADR-0023** — Performance and cost tuning wave (eleven decisions): jedi `_ResolveOutcome` NamedTuple removes the second `Script` per unresolved edge; NumPy dense matrix PageRank replaces pure-Python power iteration; SCC-count fast-path skips `simple_cycles` on DAGs; ingestion `rglob("*.py")` pre-filter; `planning.max_outline_edges` cap (default 200) with PageRank-sum ranking; three-tier research-notes truncation (`narration.research_notes_cap_kb` / `research_notes_summarize_threshold_kb` / `summarize_model`); planning retry slim message (drop outline on attempts ≥ 1); `run_agent` inner-call retry coverage with extended Anthropic exception set; SQLite PRAGMAs (`synchronous=NORMAL` + `cache_size=-32000` + `temp_store=MEMORY`); per-block `highlight_python_lines` with singleton lexer/formatter (BREAKING internal API); `scripts/validate_cost_estimator.py` research tooling for preflight-vs-real-spend delta. (2026-05-17, v0.12.0 BREAKING — only the `highlight_python` adapter helper, no external API impact).
