...
"""Tests for ``StaticPricingCatalog`` (ADR-0014 / ADR-0020 — pricing port)."""
...
def test_returns_known_anthropic_prices() -> None:
    cat = StaticPricingCatalog()
    assert cat.prices_per_mtok("claude-opus-4-7") == MODEL_PRICES["claude-opus-4-7"]


def test_returns_known_openai_prices() -> None:
    cat = StaticPricingCatalog()
    assert cat.prices_per_mtok("gpt-4.1") == MODEL_PRICES["gpt-4.1"]


def test_unknown_model_returns_none() -> None:
    cat = StaticPricingCatalog()
    assert cat.prices_per_mtok("not-a-real-model") is None


def test_custom_price_map_overrides_default() -> None:
    cat = StaticPricingCatalog(prices={"my-llama": (0.05, 0.10)})
    assert cat.prices_per_mtok("my-llama") == (0.05, 0.10)
