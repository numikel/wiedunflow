...
def test_estimate_applies_safety_factor() -> None:
    """Formula: (symbols * 500 * haiku_price + lessons * 8000 * sonnet_price) * 1.3."""
    result = estimate(symbols=100, lessons=10, clusters=2)
    # haiku: 100 * 500 = 50_000 tokens -> 50_000/1e6 * 0.80 * 1.3 = 0.052
    assert result.haiku_tokens == 50_000
    assert abs(result.haiku_cost_usd - 0.05) < 0.01
    # sonnet: 10 * 8000 = 80_000 tokens -> 80_000/1e6 * 3.00 * 1.3 = 0.312
