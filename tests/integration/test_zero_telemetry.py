...
def test_cli_runs_under_network_namespace(tmp_path: Path) -> None:
    ...
    CODEGUIDE_LLM_PROVIDER=anthropic + --no-consent-prompt.  The CLI is
...
@pytest.mark.integration
def test_generated_html_contains_codeguide_footer(
    tiny_repo_copy: Path,
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """Generated tutorial.html must include 'Generated by CodeGuide' in the footer."""
    monkeypatch.setattr(
        "wiedunflow.cli.main._build_llm_provider",
        lambda config, **_kwargs: FakeLLMProvider(),
    )
    ...
    assert "CodeGuide" in html_content, (
        "Generated HTML must mention 'CodeGuide' in the footer or metadata"
    )
