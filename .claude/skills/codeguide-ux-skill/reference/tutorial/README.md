# Handoff: CodeGuide Tutorial Reader

## Overview

This handoff bundles the design for **CodeGuide's generated `tutorial.html`** — the reader-facing HTML artifact that every `codeguide init` run produces. CodeGuide is a CLI that analyses a Python repository and emits a self-contained, offline HTML tutorial; this document describes what that HTML should look like, behave like, and contain.

The goal of this handoff is to let a developer recreate the design **inside the CodeGuide Python renderer** (Jinja2 templates + static CSS + a small amount of vanilla JS, per the tech stack). No SPA framework is required — the output is a single self-contained HTML file that must work when opened via `file://`.
...
- **Renderer:** Jinja2 templates invoked from the CodeGuide Python CLI.
...
- Contents (horizontal flex, gap 16, px 20):
  - **Brand**: 22×22 square, `border-radius: 5px`, background `--ink`, color `--bg`, mono font, label "cg" + "CodeGuide" in 13px/600 Inter
...
- Offline line: "Generated by CodeGuide vX.X.X (Apache 2.0) — this document is fully offline."
...
- **Persistence**: write current lesson id to `localStorage` key `codeguide:<repo>:last-lesson`. Read on load.

### Splitter drag
- `mousedown/touchstart` on splitter → `dragging = true`, `body.classList.add('is-resizing')`
- `mousemove/touchmove` → clientX relative to content rect → frac = clamp(0.28, 0.72, x/width). Apply `content.style.setProperty('--narr-frac', frac)`; position splitter at `frac * 100%`. Persist to `codeguide:tweak:narr-frac:v2`.
...
- `codeguide:<repo>:last-lesson` — last open lesson id (string)
- `codeguide:tweak:palette:v2` — `a1`|`a2`|`a3`
- `codeguide:tweak:dir:v2` — `A`|`B`
- `codeguide:tweak:theme:v2` — `light`|`dark`
- `codeguide:tweak:font:v2` — `sans`|`serif`|`mono`
- `codeguide:tweak:conf:v2` — `high`|`medium`|`low` (prototype-only; real reader reads this from the manifest)
- `codeguide:tweak:deg:v2` — `off`|`on` (prototype-only; real reader reads `run_status` from the manifest)
- `codeguide:tweak:narr-frac:v2` — number in [0.28, 0.72]
...
TUTORIAL_META = {
  ...
  codeguide_version, schema_version,
...
- [ ] Move the stylesheet into CodeGuide's Jinja templates folder (`codeguide/renderer/templates/`), inline it into the final HTML via `{% include %}` or direct interpolation so there are no external CSS files at runtime.
