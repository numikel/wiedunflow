/* CodeGuide tutorial reader — interactive prototype */
(function () {
  ...
  // --- State ---
  const STORAGE_KEY = "codeguide:requests:last-lesson";
  const DIR_KEY = "codeguide:tweak:dir:v2";
  const THEME_KEY = "codeguide:tweak:theme:v2";
  const FONT_KEY = "codeguide:tweak:font:v2";
  const CONF_KEY = "codeguide:tweak:conf:v2";
  const DEG_KEY = "codeguide:tweak:deg:v2";
  const PAL_KEY = "codeguide:tweak:palette:v2";
  // Clear old v1 keys once
  ["codeguide:tweak:dir","codeguide:tweak:theme","codeguide:tweak:font","codeguide:tweak:conf","codeguide:tweak:deg"].forEach(k => localStorage.removeItem(k));
  ...
  function renderFooter() {
    ...
    const degraded = localStorage.getItem(DEG_KEY) === "on";
    const version = META.codeguide_version;
    ...
    document.getElementById("tutorial-footer").innerHTML = `
      <div>
        ...
        <div class="offline">Generated by CodeGuide v${version} (Apache 2.0) — this document is fully offline.</div>
      </div>
  ...
  function setupSplitter() {
    ...
    const FRAC_KEY = "codeguide:tweak:narr-frac";
