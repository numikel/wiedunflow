...
CodeGuide merges configuration from up to six sources before passing a
validated `CodeguideConfig` to the pipeline.  Sources are consulted in a
...
| 5        | User-level config (see path table below)            | run `codeguide init`                         |
...
| OS           | Default path                                                    |
|--------------|-----------------------------------------------------------------|
| Linux / macOS| `~/.config/codeguide/config.yaml`                               |
| Windows      | `%APPDATA%\codeguide\config.yaml`                               |

Path is resolved via `platformdirs.user_config_dir("codeguide")`.
...
└─────────────────────────┘
           ...
           ▼
┌─────────────────────────┐
│  2. Env variables        │  CODEGUIDE_LLM_PROVIDER set?
└─────────────────────────┘
           │ not set
           ▼
┌─────────────────────────┐
...
└─────────────────────────┘
           ...
           ▼
┌─────────────────────────┐
│  5. User-level config    │  ~/.config/codeguide/config.yaml?
└─────────────────────────┘
           │ absent
           ▼
┌─────────────────────────┐
...
Pydantic BaseSettings reads `CODEGUIDE_*` env vars automatically
(`env_prefix = "CODEGUIDE_"`).  Most fields map 1-to-1:

| Env var                      | Config field          |
|------------------------------|-----------------------|
| `CODEGUIDE_LLM_PROVIDER`     | `llm_provider`        |
| `CODEGUIDE_LLM_MODEL_PLAN`   | `llm_model_plan`      |
| `CODEGUIDE_LLM_MODEL_NARRATE`| `llm_model_narrate`   |
| `CODEGUIDE_LLM_CONCURRENCY`  | `llm_concurrency`     |
| `CODEGUIDE_LLM_MAX_RETRIES`  | `llm_max_retries`     |
| `CODEGUIDE_LLM_MAX_WAIT_S`   | `llm_max_wait_s`      |
| `CODEGUIDE_LLM_API_KEY`      | `llm_api_key`         |
| `CODEGUIDE_LLM_BASE_URL`     | `llm_base_url`        |
...
merged YAML dict before `CodeguideConfig` is constructed, ensuring env always
...
```
ts=... level=debug logger=codeguide.cli.config msg="config resolved: llm_provider=openai from cli"
ts=... level=debug logger=codeguide.cli.config msg="config resolved: llm_model_plan=gpt-4o from yaml"
ts=... level=debug logger=codeguide.cli.config msg="config resolved: llm_model_narrate=claude-opus-4-7 from default"
```
...
Source: `src/codeguide/cli/config.py::load_config`
