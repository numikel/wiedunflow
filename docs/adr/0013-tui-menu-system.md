# ADR-0013: Interactive menu-driven TUI ("centrum dowodzenia") for `wiedun-flow`
...
WiedunFlow v0.2.x was a one-shot CLI: `wiedun-flow init` (5-step Click wizard) and `wiedun-flow generate <repo>` (7-stage pipeline with Rich animations). After UX review on 2026-04-25 the user requested a **menu-driven "command center"** at the entry point — the same pattern shipped by GitHub Copilot CLI, OpenCode, GSD (`Choice [1]:`), and Claude Code's custom-agent picker (numbered list with `↑↓ Enter Esc`). Users of AI tools expect "klikalność" — arrow navigation, Enter, Esc — not memorizing flags.

Two design alternatives were considered and rejected during planning:

1. **Persistent REPL with slash commands** (Claude Code / OpenCode style). Rejected: fragments user attention across the prompt, the scrollback, and a status bar; over-fits a power-user workflow when the primary persona is "developer trying WiedunFlow for the first time". Modal sub-wizards dominate the actual work (provider setup, repo picking) — a REPL adds machinery without UX payoff.
...
1. **Hybrid mode**: the menu activates only when `wiedun-flow` is invoked with no arguments AND `sys.stdin.isatty()` AND `sys.stdout.isatty()` AND `WIEDUNFLOW_NO_MENU` is unset. Every other invocation (`wiedun-flow generate <repo>`, `wiedun-flow init`, `wiedun-flow --version`, non-TTY pipes, `--help`) falls through to the existing Click group bit-exact.
   - Rationale: the Sprint 7 release-gate `pytest -m eval` workflow runs `subprocess.run(["wiedunflow", "generate", ...])` and depends on the existing exit codes and flag behavior. Breaking that contract would break CI.
...
9. **`target_audience` is a Literal 5-level enum** (`noob`, `junior`, `mid`, `senior`, `expert`), default `mid`. **BREAKING CHANGE** in `WiedunflowConfig` — previously a free-text `str`. Legacy YAML configs are loaded via a fuzzy-mapping shim in `_load_yaml_flat` (`"mid-level Python developer"` → `mid` with a logged warning). The shim is removed in v1.0.
...
- Model lists stay current without WiedunFlow releases.
...
- Bytecode pipelines that read `~/.cache/wiedunflow/models-*.json` files will see new 24-hour cache entries — the directory was previously empty for model lists.
...
- v0.5.0+: stale-cache warning banner when `~/.cache/wiedunflow/models-*.json` is older than 24h and a refetch is in flight.
