# ADR-0013: Interactive menu-driven TUI ("centrum dowodzenia") for `wiedunflow`
...
1. **Hybrid mode**: the menu activates only when `wiedunflow` is invoked with no arguments AND `sys.stdin.isatty()` AND `sys.stdout.isatty()` AND `WIEDUNFLOW_NO_MENU` is unset. Every other invocation (`wiedunflow generate <repo>`, `wiedunflow init`, `wiedunflow --version`, non-TTY pipes, `--help`) falls through to the existing Click group bit-exact.
