...
1. User runs `wiedun-flow generate /path/to/repo` → error: `"OpenAI API key not found. Set OPENAI_API_KEY or use --provider anthropic with ANTHROPIC_API_KEY."`
2. **Option A (recommended)** — Set `OPENAI_API_KEY` and retry. Cost-effective path.
3. **Option B (preserve Anthropic)** — Run `wiedun-flow init --provider anthropic` → wizard prompts for Anthropic key → config updated to explicit `llm.provider: anthropic`.
...
ANTHROPIC_API_KEY=sk-ant-... wiedun-flow generate repo --yes

# v0.7.0 (OpenAI explicit)
OPENAI_API_KEY=sk-... wiedun-flow generate repo --yes
# or
ANTHROPIC_API_KEY=sk-ant-... WIEDUNFLOW_LLM_PROVIDER=anthropic wiedun-flow generate repo --yes
```
