# MVP: Codebase Tutorial Generator (WiedunFlow)
...
> **Rebranding (2026-04-26):** Projekt został nazwany "WiedunFlow" (Wiedun = staropolskie określenie mędrca/sage'a; Flow = control flow + narrative flow + reader flow state). Brand jest używany wszędzie — CLI command, package, ENV prefix, localStorage keys, output filename, state directory.
...
Narzędzie CLI w Pythonie (**WiedunFlow**), które na podstawie lokalnego repozytorium generuje
...
1. Developer instaluje narzędzie (`uv tool install wiedunflow`)
2. Ustawia API key w env vars (`ANTHROPIC_API_KEY`)
3. Odpala `wiedun-flow ./moje-repo`
4. Widzi progress bar: analiza statyczna → graf relacji → RAG indexing →
   generacja lekcji
5. Po 10-30 minutach dostaje plik `wiedunflow-moje-repo.html` (~3-8 MB)
...
- **Output format**: pojedynczy self-contained HTML file (`wiedunflow-<repo>.html`), otwierany
...
- **LLM**: SDK direct (bez LangChain/LangGraph) z BYOK — user dostarcza własny API key
  przez env var. Wsparcie dla:
  - **Anthropic** (default, Haiku 4.5 + Opus 4.7) przez
    `anthropic` SDK
  - **OpenAI** (gpt-4.1) przez `openai` SDK
  - **OSS modele** (Ollama, LM Studio, vLLM) przez httpx-based OpenAI-compatible client —
    udokumentowane w README z przykładami configu
  - UWAGA: dla providerów z niestandardowymi polami (OpenRouter,
    DeepSeek, reasoning models) wymagana dedykowana implementacja
    w v2
...
- Provider-specific features (OpenRouter reasoning, DeepSeek
  reasoning_content) — wymaga dedykowanych implementacji
...
```
[CLI: user odpala wiedun-flow na ścieżce repo]
         │
         ▼
┌─────────────────────────────────────────┐
...
│ Direct SDK orchestrator ze state:       │
│   {explored_symbols,                    │
│    lessons_generated,                   │
│    concepts_introduced}                 │
│                                         │
...
│ - Inline: CSS, vanilla JS (bez          │
│   Preact/React), dane lekcji jako       │
...
<body>
  ...
  <script type="application/json" id="tutorial-data">
    {
      "metadata": {"project_name": "...", "generated_at": "...", "commit_hash": "..."},
      "lessons": [...],
...
| Orkiestracja LLM | Direct SDK (bez LangChain) + BYOK multi-provider | bezpośrednia kontrola, zero frameworków, wbudowana obsługa Anthropic/OpenAI/OSS |
| RAG storage | BM25 (rank_bm25) | lokalny, zero infra, działa wewnątrz artefaktu cache. sqlite-vec → v2 |
| Cache | SQLite + file-level hash | 80% zysku za 20% pracy, graph-level invalidation to v2 |
| Frontend output | Vanilla JS + Jinja2 template | żadnego Reacta/Astro/bundlera — constraint file:// zabrania fetch() |
| Syntax highlighting | Pygments (Python) | nie wymaga Node.js subprocess, jakościowo wystarczy |
| Model routing | Haiku 4.5 (opisy liściowe) + Opus 4.7 (narracja) | ~$0.50-$2.00 per tutorial dla średniego repo |
...
llm:
  provider: anthropic         # anthropic | openai | openai_compatible | custom
...
tutorial:
  ...
  target_audience: "mid"      # noob | junior | mid | senior | expert
...
- **Narrative coherence**: lekcja N nie może powtarzać tego, co było w
  lekcjach 1..N-1. Wymaga structured state, nie można tego
...
| Narrative coherence słabnie przy >20 lekcjach | Wysokie | Wysoki | Structured state, explicit "concepts_introduced" w promptach |
| HTML output za duży (>20MB) | Średnie | Średni | Limit na rozmiar repo w MVP (np. <500 plików), lazy rendering lekcji |
| Rate limits API | Średnie | Średni | Exponential backoff, konfigurowalny concurrency (default 10) |
...
- **Tydzień 2**: Jedi call graph + BM25 RAG + PageRank. Bez LLM
  jeszcze, tylko "structured data out".
- **Tydzień 3**: Direct SDK orchestration + LLM content generation +
...
| LLM providers | Anthropic default, OpenAI + OSS (Ollama/LM Studio) przez podmianę endpointu w HTTP. Udokumentowane w README z przykładami configu. |
...
| Output filename | Domyślnie `wiedunflow-<repo>.html`. `<repo>-tutorial-<commit>.html` to opcjonalna flag. |

### Otwarte pytania do sesji PRD

- **Distribution**: PyPI czy też Docker image dla tych co nie
  chcą instalować Pythona lokalnie?
- **CLI UX detale**: jak wygląda dokładnie flow pierwszego użycia?
  Interactive setup wizard czy tylko flagi i env vars?
- **Rozmiar repo — hard limit?**: czy narzędzie powinno odmówić dla
  repo >N plików (np. >1000), czy próbować i pokazać ostrzeżenie?
...
- **Private repo / sensitive code**: czy w MVP dokumentujemy
  ostrzeżenie "twój kod jest wysyłany do providera LLM"?
...
1. **Patent grant (sekcja 3 licencji)** — contributorzy udzielają
   ...
   ta ochrona jest istotna dla użytkowników i kontrybutorów.
...
3. **Preferowana przez enterprise** — korporacyjne legal reviews często
   ...
   clauses. Dla projektu celującego w enterprise adoption, to konkretna zaleta.

### Operacyjne wymagania (koszty)

**One-time setup (~30 min na początku projektu):**

- **`LICENSE` file** — standardowa treść Apache 2.0
- **`NOTICE` file** — plik w root zawierający copyright attribution
- **Header w plikach źródłowych** (rekomendowane):
  ```python
  # Copyright 2026 [Autor]
  # Licensed under the Apache License, Version 2.0
  ```
- **`pyproject.toml`** — pole `license = "Apache-2.0"`

**Ongoing operational costs:**

- **Dependency tracking** — jeśli dodajesz Apache-licensed dependency
- **DCO (Developer Certificate of Origin)** — sign-off w commit message
